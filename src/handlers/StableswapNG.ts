...
//
// exchange_underlying routes a trade through the metapool's base pool, so this
// event's sold_id/bought_id are UNDERLYING indices: 0 = the metapool's own
// coin[0], 1.. = the BASE pool's coins. getMetapoolUnderlyingCoins resolves that
// flattened [coin0, ...baseCoins] list so we can map indices → real tokens. We
// mirror the regular handler's volume/count bookkeeping but do NOT event-source
// balances/TVL: an underlying trade's amounts are in base-coin units, not the
// metapool's own coin[1] (base-LP) balance, so applying them as deltas is wrong.
indexer.onEvent(
  { contract: "StableswapPool", event: "TokenExchangeUnderlying" },
  async ({ event, context }) => {
    const pool = await ensureStablePool(
      event.chainId,
      event.srcAddress,
    ...
    // symbol on-chain like ensureStablePool does for a pool's own coins).
