...
// (0x8F942C20) but deployed manually before the crypto factory, so they aren't
// caught by any factory. Same old 5-field TokenExchange signature as the legacy
// tricrypto pool (no fee, no packed_price_scale). 2-coin; registered by static
// address per chain; initialized lazily on the first event.
const N_COINS = 2;
...
indexer.onEvent(
  { contract: "LegacyCryptoPool", event: "TokenExchange" },
  async ({ event, context }) => {
